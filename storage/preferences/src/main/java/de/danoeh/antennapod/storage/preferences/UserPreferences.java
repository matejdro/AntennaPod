...
public abstract class UserPreferences {
    ...
    public static float getPlaybackSpeed() {
        if (isPerOutputSpeedEnabled()) {
            ...
            return getPerOutputPlaybackSpeed(mediaRouter.getSystemController().getSelectedRoutes().getFirst().getName().toString());
        }
    ...
    public static float getPerOutputPlaybackSpeed(String mediaOutput) {
        String filteredName = mediaOutput.toLowerCase().replaceAll("[^a-z]", "");
    ...
    public static void setPlaybackSpeed(float speed) {
        if (isPerOutputSpeedEnabled()) {
            MediaRouter2 mediaRouter = MediaRouter2.getInstance(context);
            setPlaybackSpeedPerOutput(mediaRouter.getSystemController().getSelectedRoutes().getFirst().getName().toString(), speed);
            return;
        }
